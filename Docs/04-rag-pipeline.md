...
Stage 2: Semantic Extraction
    ...
    └─> **Deduplication:** After parsing all Stage 2 results, chunks are deduplicated by exact `source_content` string match. Duplicate analysis is removed — only unique source content proceeds to Stage 3.
    │
    ▼
Stage 3: RAG Chunking
    Input: each Stage-2 chunk with document context + section metadata
    Output: final chunks[] with id (globally unique `segX-NNN`), content, retrieval_content, metadata
...
| `promptDir` | `"./Prompts"` | Directory containing prompt template files |
...
**Deduplication:** After all Stage 2 results are parsed, the processor deduplicates by exact `source_content` string match. Duplicate analyses are removed before proceeding to Stage 3. This prevents redundant processing of identical source content across segments.
...
- For each semantic group from Stage 2:
  ...
  - **Globally unique IDs:** Each chunk receives a deterministic ID (`segX-NNN`) generated by a C# counter — not UUIDs. These are used as `point_string_id` in Qdrant payloads.
  - **Two-content fields:** Each chunk gets both `content` (verbatim source for embedding) and `retrieval_content` (structured: doc title + section path + topic + content for search context). If the LLM omits `retrieval_content`, it is generated from doc title + section path + topic + content.
  - **DATA LOSS CHECK:** Each chunk's content is verified against source markdown before emission to ensure no truncation or rewriting occurred.
...
```json
{
  ...
  "chunks": [
    {
      "id": "project-structure-seg0-001",
      "content": "... verbatim source markdown ...",
      "retrieval_content": "Project Structure » Processors: AgenticChunkingProcessor — topic: pipeline orchestration — content excerpt...",
      "metadata": {
...
**Key fields:**
- `id` — Globally unique, deterministic (`segX-NNN` format), assigned by C# counter not LLM. Used as Qdrant `point_string_id`.
- `content` — Verbatim source text extracted from markdown headings. Used for embedding generation.
- `retrieval_content` — Structured search context: `Doc Title » Section Path — topic: ... content excerpt...`. Passed to Qdrant payload alongside content.
...
- `02 Semantic.md` — Semantic analysis: topic extraction, entity identification, keyword generation; outputs a MANDATORY HEADING ENUMERATION block appended to final output
