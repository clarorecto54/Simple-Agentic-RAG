...
When no output path is given the app appends `.embedded.json` to the input filename. The pipeline processes chunks sequentially with a 5-minute HTTP timeout per request.
