...
The `setup_qdrant.py` script has been updated to use a relative path (`./output.json`) instead of an absolute path, making it more portable. When running the script, ensure the `.embedded.json` file exists in the working directory, as this is the actual output file generated by the C# application.
