...
16. **Modifying prompt markdown files (`[RAG] 0*.md`) requires syncing to `prompts.json`** — the compiled assembly resource loads from `prompts.json` first, falling back to filesystem `.md` files second. After editing any `[RAG] *.md`, you MUST also apply the same changes to `Prompts/prompts.json` (the embedded resource). Use a JSON editor or script — manual patching is error-prone due to escape sequences.
